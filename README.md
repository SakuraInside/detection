...
Видеоаналитика на YOLOv11, целиком на C++ / Rust. Python в рантайме не
используется — `run.py` это только launcher нативных процессов.

Два независимых контура восприятия (см. [docs/architecture.md](docs/architecture.md)):

- **люди** — class-based: YOLO cls=0 → **ByteTrack**, устойчивые track ID;
- **остальные предметы** — class-agnostic: выявляются по изменениям сцены
  (`FrameDiffDetector`), а не по именам YOLO-классов.

События определяются поведением во времени:

- **`person_interaction`** — человек подошёл к объекту;
- **`object_left`** — объект оставлен, владелец отошёл;
- **`object_unattended`** — бесхозный объект дольше порога → тревога;
- **`object_removed`** — объект исчез после взаимодействия (забрали);
...
└────────────────────────────┬─────────────────────────────────────────────┘
   ...
   │  HW-accel decode   │         │  • ByteTracker (люди, cls=0)          │
   │  → BGR frames      │         │  • FrameDiffDetector + IouTracker     │
   └────────────────────┘         │    (class-agnostic объекты сцены)     │
                                  │  • SceneAnalyzer (поведенческая FSM)  │
...
{"type":"event",  "stream_id":"main", "event":{"type":"object_unattended", "snapshot_path":"/logs/snapshots/main/...jpg", ...}}
```
...
```json
{
  ...
  "model": {
    ...
    "tracker": {
      ...
      "new_thresh": 0.60, "match_thresh": 0.80, "track_buffer": 30
    }
  },
  "analyzer": {
    "track_only_persons": true,
    "static_displacement_px": 7,
...
`object_classes` пуст: предметы — class-agnostic (контур M3), а не классы YOLO.
`model.tracker` — параметры ByteTrack (контур людей).
