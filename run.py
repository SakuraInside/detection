...
Использование:
    ...
    python run.py --release --infer-worker-addr 127.0.0.1:9910  # FFI в отдельном infer_worker (~80 МБ gateway)
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--infer-worker-addr",
        default="",
        help="адрес infer_worker (TCP). Если задан — FFI загружается в infer_worker, а не в gateway. "
             "Пример: 127.0.0.1:9910. По умолчанию пусто (прямой FFI в gateway).",
    )
    parser.add_argument(
        "--infer-worker-wait-sec",
        type=float,
        default=60.0,
    ...
    infer_proc: subprocess.Popen | None = None
    ...
    if args.infer_worker_addr:
        ...
        if not _wait_tcp_listening(args.infer_worker_addr, timeout_sec=args.infer_worker_wait_sec):
            print(
                f"[run.py] За {args.infer_worker_wait_sec}s infer_worker {args.infer_worker_addr} не открылся.\n"
                "         Проверьте логи infer_worker (CUDA, integra_ffi).",
                file=sys.stderr,
            )
            if infer_proc.poll() is not None:
                print(
                    f"[run.py] infer_worker завершился с кодом {infer_proc.returncode}.",
                    file=sys.stderr,
                )
            try:
                infer_proc.terminate()
                infer_proc.wait(timeout=2)
            except Exception:
                infer_proc.kill()
            if bridge_proc is not None:
                bridge_proc.terminate()
    ...
    try:
        ...
        for p in (bridge_proc, infer_proc):
            if p is not None:
                try:
        ...
        _stop_proc(bridge_proc)
        _stop_proc(infer_proc)
