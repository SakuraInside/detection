/* Integra-LOST front-end. No build step, plain ES2020. */
...
const state = {
  ...
  fps: 30,
  seekDragging: false,
  alarms: new Map(), // track_id -> alarm payload
  ws: null,
  settings: null,
};
...
function applyInfo(info) {
  ...
  const s = info.stats || {};
  $("#hud-decode").textContent = `decode ${(s.decode_fps || 0).toFixed(0)} fps`;
  $("#hud-render").textContent = `render ${(s.render_fps || 0).toFixed(0)} fps`;
  $("#hud-inf").textContent = `inf ${(s.inference_ms_avg || 0).toFixed(1)} ms`;
  $("#hud-events").textContent = `events ${s.events || 0}`;
  $("#dropped-info").textContent = `пропущено ${s.dropped_decode || 0}`;

  // Sync alarm map with track snapshot.
  ...
  // Drop alarms whose tracks are gone (analyzer pruned them).
  ...
  for (const a of state.alarms.values()) {
    ...
    el.innerHTML = `
      <div>
        ...
        <div class="text-[11px] text-slate-500">${a.state}</div>
      </div>
      <span class="${badgeClass}">ALARM</span>
    `;
    root.appendChild(el);
  }
}
...
async function openVideo(path) {
  try {
    ...
    await refreshInfo();
    await refreshEvents();
    // Force the MJPEG <img> to reconnect.
    const v = $("#video");
